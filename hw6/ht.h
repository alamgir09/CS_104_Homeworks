...
// Almost Complete - Fill in the if statement below. - attempted
template <typename KeyType>
struct LinearProber : public Prober<KeyType> {
    ...
    HASH_INDEX_T next() 
    {
        ...
        if(this->numProbes_ == this->m_) { // changed this
            return this->npos; 
        }
...
public:
    ...
    HASH_INDEX_T next() 
    {
        // just gives next loc in the table to look
        // this is for DOUBLE HASH PROBER
        // you will use double hash probing, but the (if conditions) for number probes will be the same as linear probing, because the size of the table is still prime

        // this needs to be adjusted for double hashing vv
...
private:
    ...
    int size_; // number of non-deleted
    int capacity_; // total number of entries (counts non-deleted), will be used for loading factor
    // int tableIndex_; // index we're at for the prime table size
    double resizeAlpha; // just the loading alpha we keep comparing against
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    // When constructed, start with size of 11
    size_ = 0;
    capacity_ = 0;
    //std::vector<V> hashVector (CAPACITIES[0], 0);
    // do we need to initialize table_
    mIndex_ = 0;
    resizeAlpha = this->resizeAlpha;
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
    // Destroy the Hash Table object and delete all remaining
    // key,value pairs
    for (auto i : table_)
    {
        delete i;
...
{
    // just checks if the hashtable is empty and has no non-deleted keys
    // iterate through the hasthable
    // just use size function, if it equals 0 then it's empty
    return (size_== 0);
...
{
    // an alpha is given before inserting any new element you should call resize (to next larger size in the list) if the current loading factor is at or above the given alpha
    // simply hash the key and allocate a new HashItem (vector) and free them at an appropriate time
    // we will simply take the more inefficient probing approach of not reusing locations marked for deletion when probing for insertion.
    // everytime you insert increment size_ by 1
    if ((capacity_/CAPACITIES[this->mIndex_]) >= resizeAlpha)
    {
        mIndex_++;
        resize();
    }
    ...
    // or does the code do that for us? I think it does it for us
    HashItem* newHash = new HashItem(p);
    if (h == npos) throw std::out_of_range("Can't insert at location");
    else if (table_[h] != nullptr && table_[h]->deleted == false)
    {
...
{
    // find if the key given to exists and gives an location, if not just return
    // if it exists then change bool to deleted
    if (empty()) return;
    HASH_INDEX_T h = this->probe(key);
...
{
    // change current size to the next size in the list of prime sizes given to us
    // so essentially double the size, and then rehash all non-deleted keys to the next vector hashtable
    
    std::vector<HashItem*> oldVector;

    for (int i = 0; i < CAPACITIES[mIndex_];i++)
    {
        oldVector.push_back(table_[i]);
    }
    // create new table same size as old, then make the old table empty and bigger, then simply use probe to find the new index
    for (auto i : table_)
    {
        delete i;
    }
    ...
    size_ = 0;
    capacity_ = 0;
    // now iterate through the old HashTable and hash the non-deleted keys to the new index in the newVector
    // don't forget about changing capacity and size to 0, or actually just make capacity equal to 0

    for (int i = 0; i < (int)CAPACITIES[mIndex_-1]; i++)
    {
        if (oldVector[i]->deleted == true)
        {
            delete oldVector[i];
            oldVector[i] = nullptr;
        }
        else if (oldVector[i] != nullptr)
        {
            HASH_INDEX_T newH = this->probe(oldVector[i]->item.first);
            if (newH == npos) throw std::out_of_range("Can't insert at location");
            else
            {
                table_[newH] = oldVector[i];
                size_++;
                capacity_++;
                delete oldVector[i];
                oldVector[i] = nullptr;
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if(loc == h && table_[loc]->deleted == false) { // changed this
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
